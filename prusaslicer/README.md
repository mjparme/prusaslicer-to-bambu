...
**You do not need to load these files manually.** The G-code is already embedded in the printer profiles at the repo root (`config-a1-*.ini`) as `start_gcode` and `end_gcode`. Import a config and PrusaSlicer uses that inline G-code automatically.
