...
**Imported preset names:** PrusaSlicer prefixes each imported config with its filename, so profiles appear as `config-a1-smooth.ini (Bambu Lab A1)` rather than just `Bambu Lab A1`. This is normal; it avoids name collisions. The presets work the same. To clean up the names, save each preset under a new name (disk icon) and delete the prefixed copies.

**Expected import warning:** PrusaSlicer shows an alert that the config contains a post-processing script and asks you to review it before exporting G-code. So **read `scripts/prusaslicer-to-bambuddy.py` in this repo first** (it wraps G-code for BamBuddy upload; see step 4), then click **OK** once you are satisfied with what it does.
